...
int main(int argc, char * argv[]) 
{
    ...
    }
    {
        ...
        // auto& v2 = func_ccptr();    // 6.2 -v2 type ?  invalid initialization of non-const reference of type 'const int*&' from an rvalue of type 'const int*'
        const auto& v3 = func_ccptr();  // 6.3 -v3 type ? const int* cons
        auto&& v4 = func_ccptr();   // 6.4 -v4 type ? const int* 
    }
    {
        auto v1 = func_rref();     // 7.1 -v1 type ? int
        // auto& v2 = func_rref();    // 7.2 -v2 type ?  invalid initialization of non-const reference of type 'int&' from an rvalue of type 'int'
    ...
    }
    {
        ...
        auto v3{10};    // 8.3 -v3 type ? std::initializer_list<int>
        auto v4 = {10};     // 8.4 -v4 type ? std::initializer_list<int>
        auto v5 = {1, 2, 3};    // 8.5 -v5 type ? std::initializer_list<int>
        // auto v6 = {1, 2, 3.0};  // 8.6 -v6 type ? unable to deduce 'std::initializer_list<auto>' from '{1, 2, 3.0e+0}'
    }
    return 0;
}
